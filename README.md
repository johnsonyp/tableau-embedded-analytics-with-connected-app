# Tableau Embedded Analytics with Connected App

This is a sample of how you can use [Tableau Embedded Analytics](https://www.tableau.com/products/embedded-analytics) in conjunction with [Tableau Connected Apps](https://help.tableau.com/current/online/en-us/connected_apps.htm).

For this demo, we will use **Python** and `Flask` to create both the front and backend for our application.

Simply run `app.py` in `src/api` to deploy the demo application locally. You should be able to access the app via [localhost:8080](http://localhost:8080) or http://127.0.0.1:8080/ as prompted in the terminal.

## Overview

While embedding a Tableau dashboard is very straightforward in that the embed code snippet can be easily pulled when you use the *share* option from a dashboard, we can run into some real challenges when it comes to wanting to create a more seamless experience.
```html
...
The primary use case that comes to mind when needing to integrate with a Connected App is to prevent the need to "sign-in" multiple times. For instance, you have your own website or application which users are already accessing via some sign-in methods, an emedded Tableau dashboard will likely prompt them to sign-in via their Tableau account for viewing. This can lead to a poor user experience, especially if you are servicing clients and not just internal staff.

Hence, we need Tableau's **Connected App** to help your application communicate to Tableau that this user is already authorized and permitted to view whatever dashboard you have embedded. 
...
The arguments passsed in `app.py` to generate the token will pull from the `config/settings.py` which loads configuration values from the `.env` file.
...
Again, you can test all this by simply running the `app.py` and going to http://127.0.0.1:8080/ via a browser.
...
### Settings
In `app.py`, key credentials are loaded from environment variables. Ensure your Tableau instance values are defined in the `.env` file.
